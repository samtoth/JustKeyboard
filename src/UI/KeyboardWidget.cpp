...
void KeyboardWidget::paintEvent(QPaintEvent *e) {
    ...
    for (int i = 0; i < 13; i++) {
        ...
        for (int j = 1; j < 12; j++) {
            ...
            p.drawText(cW + bar/2 - 5, b + 25, toFrac(numerators[i] * j, denominators[i]));
            p.drawText(cW + bar / 2 - 5, b + ((ratios(i) * 0.5f) * h / 10),
                       QString::fromStdString(getNoteName(fundFreq * ratios(i) * j, &midiCentDiff)));
            p.drawText(cW + bar / 2 - 5, b + ((ratios(i) * 0.7f) * h / 10), QString::number(midiCentDiff));
        }

        cW += bar / 2;
        p.drawText(cW - 5, h + 20, labels[i]);
        cW += bar / 2;
